<div>
    <div class="network-switcher dropdown inline-block relative" x-data="{ open: false }" @click.away="open = false">
        ...
        <div role="button" class="bg-gray-300 font-semibold pb-2 px-4 rounded inline-flex items-center outline-none" @click="open = true">
        ...
        <ul class="dropdown-menu absolute pt-1 bg-white rounded w-full shadow pb-2 network-switcher-menu" x-show="open">
            @foreach($availableNetworks as $network)
                <li><a title="{{ $network['title'] }}" class="rounded-t bg-gray-200 hover:bg-gray-400 pt-3 pb-1 px-5 block whitespace-no-wrap theme-text cursor-pointer" wire:click="changeNetwork('{{ $network['value'] }}')" @click="open = false">{{ $network['value']}}</a></li>
            @endforeach
        </ul>
